...
/**
    ...
    await val.validate("cpf",
            v.required(),
            v.CPF(),
            v.unique({ model: Usuario, query: { cpf: req.body.cpf } })
    );
...
export class Validator {
    ...
    /**
     * Retorna um objeto com apenas os valores sanitizados que passaram pelo validador
     * Não funciona com subcampos, irá incluir o campo 'pai' inteiro, mesmo que apenas um subcampo tenha sido validado
     * (Ignorando valores undefined)
     */
    getSanitizedBody() {
        let sanitizedBody = {};
        Object.keys(this.validations).forEach((path) => {
            // Ignora subcampos, pois o campo 'pai' com ele dentro já foi incluido
...
export class ValidationFuncs {
    ...
    static toBoolean = (opcoes = {}) => async (value, val) => {
        val.setValue((value === true || value === "true") ? true : false);
        return true;
    };
    ...
    */
    static toUTCDate = (opcoes = {}) => async (value, val) => {
        let dateString;
        if(/^\d{4}-\d{1,2}-\d{1,2}$/.test(value)) { // YYYY-M-D || YYYY-MM-DD
            const parts = value.split("-");
            const year = parts[0];
            const month = parts[1].padStart(2, "0"); // Adiciona zero se necessário
            const day = parts[2].padStart(2, "0"); // Adiciona zero se necessário
            dateString = `${year}-${month}-${day}T00:00:00Z`;
        } else if(/^\d{4}-\d{1,2}$/.test(value)) { // YYYY-M || YYYY-MM
            const parts = value.split("-");
            const year = parts[0];
            const month = parts[1].padStart(2, "0"); // Adiciona zero se necessário
            dateString = `${year}-${month}-01T00:00:00Z`;
        } else {
            return opcoes.message || messages.customValidation.invalidDate;
        }
    ...
    static optional = (opcoes = {allowNull: false}) => async (value) => {
        ...
        if (!opcoes.allowNull && value === null) return false;
        else return true;
    };

    /** Se não existir irá parar as validações (com erro). */
    static required = (opcoes = {allowNull: false}) => async (value, val) => {
        if (value === undefined || (!opcoes.allowNull && value === null)) {
            return opcoes.message || messages.validationGeneric.fieldIsRequired(val.path).message;
        } else return true;
    };

    /** Se não existir continua com o valor default */
    static requiredOrDefault = (opcoes = {default: "", allowNull: false}) => async (value, val) => {
        if (value === undefined || (!opcoes.allowNull && value === null)) {
    ...
    /**
     * Verifica se o valor se está entre min e max caracteres (Deve ser uma string ou array, produz erro se o valor for undefined)
     */
    static length = (opcoes = {max: false, min: false}) => async (value, val) => {
        ...
        if (opcoes.min !== false && value.length < opcoes.min) {
            return opcoes.message || messages.customValidation.lengthTooShort(val.path, opcoes.min);
        } 

        if (opcoes.max !== false && value.length > opcoes.max) {
            return opcoes.message || messages.customValidation.lengthTooBig(val.path, opcoes.max);
        } 
    ...
    static isArray = (opcoes = {}) => async (value, val) => {
        if(!Array.isArray(value)) {
            return opcoes.message || messages.validationGeneric.invalidInputFormatForField(val.path).message;
        } else return true;
    };

    static arrayValidate = (opcoes = {validate: false}) => async (value, val) => {
        if(opcoes.validate === false) throw new Error("A função de validação arrayItemsValidate deve receber a função de validação validate");

        // Pode passar um array de funções de validação ou uma função
        let validateFuncs = Array.isArray(opcoes.validate) ? opcoes.validate : [opcoes.validate];

        for(let i = 0; i < value.length; i++) {
            for (let validateFunc of validateFuncs) {
                let resultado = await validateFunc(value[i], new ValidationResult(val.path+"."+i, val.body));

                // retorna no primeiro erro
    ...
    static isObject = (opcoes = {}) => async (value, val) => {
    ...
    static min = (opcoes = {min: false}) => async (value, val) => {
        ...
        if (value < opcoes.min) {
            return opcoes.message || messages.customValidation.valueTooSmall(val.path, opcoes.min);
        } else return true;
    };
    ...
    static max = (opcoes = {max: false}) => async (value, val) => {
        ...
        if (value > opcoes.max) {
            return opcoes.message || messages.customValidation.valueTooBig(val.path, opcoes.max);
        } else return true;
    };
